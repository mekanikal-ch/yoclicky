...
struct BlueCursorView: View {
    ...
    private let fullWelcomeMessage = "hey! i'm yoclicky"
                ...
                Text(welcomeText)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            .fill(cursorColor)
                            .shadow(color: cursorColor.opacity(0.5), radius: 6, x: 0, y: 0)
                    )
                    .fixedSize()
                    .overlay(
                ...
                Text(navigationBubbleText)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            .fill(cursorColor)
                            .shadow(
                                color: cursorColor.opacity(0.5 + (1.0 - navigationBubbleScale) * 1.0),
        ...
        Timer.scheduledTimer(withTimeInterval: 0.03, repeats: true) { timer in
            guard currentIndex < self.fullWelcomeMessage.count else {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
                    ...
                    // Start the built-in tour right after the welcome text disappears
                    self.companionManager.startOnboardingTour()
                }
                return
            }
